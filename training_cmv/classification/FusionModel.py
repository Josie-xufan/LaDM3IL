...
class BilinearFusion(nn.Module):
    def __init__(self, skip=True, use_bilinear=True, 
        ...
        self.linear_z2 = nn.Bilinear(dim1_og, dim2_og, dim2) if use_bilinear else nn.Sequential(nn.Linear(dim2_og+dim1_og, dim2))
        ...
        # self.concat_encoder = nn.Sequential(nn.Linear(dim1_og+dim2_og, mmhid), nn.ReLU(), nn.Dropout(p=dropout_rate))
    ...
    def forward(self, vec1, vec2):
        ...
        if vec2!=None:
            ...
            # # Rebuttal: 直接把两个模态concatenate起来####################################################
            # out = torch.concat((vec1, vec2), 1)
            # out = self.concat_encoder(out)
            # ############################################################################################
